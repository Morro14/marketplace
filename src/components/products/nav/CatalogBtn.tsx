...
export default function CatalogBtn({ categories }: { categories: Category[] }) {
  ...
  return (
    <div>
      <button
        onClick={handleClick}
        className="btn__accent flex items-center gap-2 px-5 bg-accent h-8 rounded-lg group"
      >
        <Image
          src={catalogBtnIcon}
          alt="catalog-btn-icon"
          className={`${catalogOpen ? "rotate-90" : "rotate-0"} transition-transform duration-300 ease-out`}
        ></Image>
        <span>{t("Catalog")}</span>
      </button>
      <CatalogSide
