...
export default function Count({ product }: { product: Product }) {
  ...
  const handleAddToCardClick = async () => {
    ...
    dispatch(setProductCount({ productId: product.id, count: nextCount, product }));
    ...
    try {
      ...
      if (error instanceof BasketApiError && error.status === 409 && error.data) {
        updateBasketProductData(dispatch, error.data);
        setInputCount(error.data.count);
      } else {
        dispatch(setProductCount({ productId: product.id, count: previousCount, product }));
